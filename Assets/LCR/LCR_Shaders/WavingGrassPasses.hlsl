...
{
    float3 worldPos = mul(unity_ObjectToWorld, float4(input.vertex.xyz, 1.0)).xyz;

    float3 displacement = float3(0, 0, 0);
    float3 direction = normalize(worldPos.xyz - kqnx_char_pos.xyz);
    //direction.y *= -1;
    //direction.y = clamp(direction.y, 0, 1);
    float horizontal_dist_factor = 1 - clamp(distance(worldPos.xz, kqnx_char_pos.xz) * 0.1, 0, 1);
    float vertical_dist_factor = clamp(distance(worldPos.y, kqnx_char_pos.y) * 0.1, 0, 1);
    displacement.xz = direction.xz * vertical_dist_factor * horizontal_dist_factor;
    displacement = mul(unity_WorldToObject, float4(displacement, 0.0)).xyz;

    input.vertex.xyz += displacement * 5;
